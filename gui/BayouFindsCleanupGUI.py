...
class BayouFindsCleanupGUI:
    ...
    def _format_printer_health_breakdown(self, report: dict, exit_code: int) -> str:
        ...
        lines = [
            ...
            f"Offline Printers: {summary.get('OfflinePrinters') if summary.get('OfflinePrinters') is not None else 0}",
            ...
            f"Status: {summary.get('PrinterHealthStatus') or ('Healthy' if isinstance(score, int) and score >= 90 else 'Warning')}",
            "",
            f"Online Printers: {summary.get('OnlinePrinters') if summary.get('OnlinePrinters') is not None else 0}",
